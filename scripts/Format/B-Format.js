class BFormat {
 ...
 // Address in hexadecimal format
        ...
        this.controlSignals = {
            ...
            ALUOp1: LEGv8Registers.valueTo64BitBinary(BFormatInstruction.definition.controlSignals.aluOp).slice(-2, -1), // ALUOp1 is the second last bit of ALUOp
    ...
    async memoryAccess() {
        ...
        const register2_hexan = getBits(instruction, 0, 4)
        const addressShifted_bin = LEGv8Registers.valueTo64BitBinary(LEGv8Registers.binaryToBigInt(LEGv8Registers.signExtend(this.address)) << BigInt(2));
        const addressShifted_decimal = Number(LEGv8Registers.binaryToBigInt(addressShifted_bin));
        const addressShifted_hexan = LEGv8Registers.binaryToHex(addressShifted_bin); // Shift left by 2 bits
