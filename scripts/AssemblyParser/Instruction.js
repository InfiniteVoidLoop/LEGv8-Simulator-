class Instruction {
    constructor(bytecode, definition) {
        ...
        if (this.bytecode.length > 32) {
            console.error(`WARNING: Bytecode provided to Instruction constructor has length ${this.bytecode.length} (> 32) for ${definition.getMnemonic()}.`);
        }
    }
    ...
    static setBits(bits, value, startBit, endBit) {
        if (startBit < 0 || endBit < startBit || endBit >= 32) {
            console.error(`WARNING: Invalid bit range for setting: ${startBit} to ${endBit}`);
            return;
        }
    ...
    static extractBits(bits, startBit, endBit) {
        if (startBit < 0 || endBit < startBit || endBit >= 32) {
            console.error(`WARNING: Invalid bit range requested: ${startBit} to ${endBit}`);
            throw new Error(`Invalid bit range requested: ${startBit} to ${endBit}`);
        }
    ...
    disassemble() {
        throw new Error('Abstract method "disassemble()" must be implemented by subclasses.');
    }
